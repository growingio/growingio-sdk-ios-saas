...
@interface GrowingMobileDebugger() <Growing3rdLibSRWebSocketDelegate, GrowingEventManagerObserver, CLLocationManagerDelegate>
...
- (NSMutableDictionary *)userInfo {
    ...
    NSString *loginId       = deviceInfo.deviceIDString;
    NSString *sdkVersion    = [Growing sdkVersion];
...
- (void)sendEventInfo:(GrowingEvent * _Nullable)event {
    ...
    NSString *eventType = eventInfo[@"t"];
    
    if([eventType isEqualToString:@"imp"]){
        ...
        urlTemplate = kGrowingEventApiTemplate_PV;
             ...
             || [eventType isEqualToString:@"pvar"]
             || [eventType isEqualToString:@"evar"]
             || [eventType isEqualToString:@"ppl"]
             || [eventType isEqualToString:@"vstr"]){
    ...
    if ([eventType isEqualToString:@"reengage"] ||
        [eventType isEqualToString:@"activate"]) {

        urlTemplate = kGrowingEventApiTemplate_Activate;
        url = kGrowingReportApi(urlTemplate, [GrowingInstance sharedInstance].accountID, stm);
    }
