...
@interface FMG3Database ()

- (FMG3ResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;

@end

@implementation FMG3Database
...
static int FMG3DBDatabaseBusyHandler(void *f, int count) {
    FMG3Database *self = (__bridge FMG3Database*)f;
...
- (void)resultSetDidClose:(FMG3ResultSet *)resultSet {
...
- (FMG3Statement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(FMG3Statement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(FMG3Statement*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
    ...
    FMG3ResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    ...
    if (_crashOnErrors) {
        NSAssert(false, @"The FMG3Database %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    #ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The FMG3Database %@ is not open.", self);
            abort();
        }
    #endif
...
- (NSError*)errorWithMessage:(NSString*)message {
    ...
    return [NSError errorWithDomain:@"FMG3Database" code:sqlite3_errcode(_db) userInfo:errorMessage];    
}
...
- (FMG3ResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    FMG3Statement *statement  = 0x00;
    FMG3ResultSet *rs         = 0x00;
    ...
    FMG3DBRetain(statement); 
    
    if (!statement) {
        statement = [[FMG3Statement alloc] init];
    ...
    rs = [FMG3ResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (FMG3ResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (FMG3ResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error {
    FMG3ResultSet *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil];
    if (!rs && error) {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[FMG3Statement alloc] init];
