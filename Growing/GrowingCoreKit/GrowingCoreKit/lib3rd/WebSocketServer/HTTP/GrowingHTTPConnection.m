...
@interface GrowingHTTPConnection (PrivateAPI)
...
@implementation GrowingHTTPConnection
...
+ (void)initialize
{
	...
	dispatch_once(&onceToken, ^{
		...
		recentNonceQueue = dispatch_queue_create("GrowingHTTPConnection-Nonce", NULL);
...
- (id)initWithAsyncSocket:(GrowingGCDAsyncSocket *)newSocket configuration:(GrowingHTTPConfig *)aConfig
{
	if ((self = [super init]))
	{
		...
		request = [[GrowingHTTPMessage alloc] initEmptyRequest];
...
{	
	...
	if ([GrowingHTTPResponse respondsToSelector:@selector(connectionDidClose)])
	{
		[GrowingHTTPResponse connectionDidClose];
	}
}
...
- (BOOL)isAuthenticated
{
	...
	GrowingHTTPAuthenticationRequest *auth = [[GrowingHTTPAuthenticationRequest alloc] initWithRequest:request];
	...
	if ([self useDigestAccessAuthentication])
	{
		...
		NSString *HA1 = [[[HA1str dataUsingEncoding:NSUTF8StringEncoding] growing_md5Digest] growing_hexStringValue];
		
		NSString *HA2 = [[[HA2str dataUsingEncoding:NSUTF8StringEncoding] growing_md5Digest] growing_hexStringValue];
		
		NSString *responseStr = [NSString stringWithFormat:@"%@:%@:%@:%@:%@:%@",
								 HA1, [auth nonce], [auth nc], [auth cnonce], [auth qop], HA2];
		
		NSString *response = [[[responseStr dataUsingEncoding:NSUTF8StringEncoding] growing_md5Digest] growing_hexStringValue];
	...
	else
	{
		...
		NSData *temp = [[base64Credentials dataUsingEncoding:NSUTF8StringEncoding] growing_base64Decoded];
...
- (void)addDigestAuthChallenge:(GrowingHTTPMessage *)response
{
...
- (void)addBasicAuthChallenge:(GrowingHTTPMessage *)response
{
...
- (void)startReadingRequest
{
	[asyncSocket readDataToData:[GrowingGCDAsyncSocket CRLFData]
...
- (BOOL)parseRangeRequest:(NSString *)rangeHeader withContentLength:(UInt64)contentLength
{
	...
	for (i = 0; i < [rangeComponents count]; i++)
	{
		...
		if (dashRange.location == NSNotFound)
		{
			...
			UInt64 byteIndex;
			if(![NSNumber growing_parseString:rangeComponent intoUInt64:&byteIndex]) return NO;
			
			if(byteIndex >= contentLength) return NO;
			
			[ranges addObject:[NSValue valueWithGrowingDDRange:GrowingDDMakeRange(byteIndex, 1)]];
		}
		else
		{
			...
			BOOL hasR1 = [NSNumber growing_parseString:r1str intoUInt64:&r1];
			BOOL hasR2 = [NSNumber growing_parseString:r2str intoUInt64:&r2];
			...
			if (!hasR1)
			{
				...
				[ranges addObject:[NSValue valueWithGrowingDDRange:GrowingDDMakeRange(startIndex, r2)]];
			}
			else if (!hasR2)
			{
				...
				[ranges addObject:[NSValue valueWithGrowingDDRange:GrowingDDMakeRange(r1, contentLength - r1)]];
			}
			else
			{
				...
				[ranges addObject:[NSValue valueWithGrowingDDRange:GrowingDDMakeRange(r1, r2 - r1 + 1)]];
			}
		}
	}
	...
	{
		GrowingDDRange range1 = [[ranges objectAtIndex:i] GrowingDDRangeValue];
		
		NSUInteger j;
		for (j = i+1; j < [ranges count]; j++)
		{
			GrowingDDRange range2 = [[ranges objectAtIndex:j] GrowingDDRangeValue];
			
			GrowingDDRange iRange = GrowingDDIntersectionRange(range1, range2);
	...
	[ranges sortUsingSelector:@selector(GrowingDDRangeCompare:)];
...
- (void)replyToHTTPRequest
{
	...
	if ([GrowingWebSocketServer isGrowingWebSocketServerRequest:request])
	{
		
		GrowingWebSocketServer *ws = [self webSocketServerForURI:uri];
		...
		else
		{
			...
			[[config server] addGrowingWebSocketServer:ws];
	...
	GrowingHTTPResponse = [self GrowingHTTPResponseForMethod:method URI:uri];
	
	if (GrowingHTTPResponse == nil)
	{
		[self handleResourceNotFound];
		return;
	}
...
- (GrowingHTTPMessage *)newUniRangeResponse:(UInt64)contentLength
{
	
	
	GrowingHTTPMessage *response = [[GrowingHTTPMessage alloc] initResponseWithStatusCode:206 description:nil version:HTTPVersion1_1];
	
	GrowingDDRange range = [[ranges objectAtIndex:0] GrowingDDRangeValue];
	
	NSString *contentLengthStr = [NSString stringWithFormat:@"%qu", range.length];
	[response setHeaderField:@"Content-Length" value:contentLengthStr];
	
	NSString *rangeStr = [NSString stringWithFormat:@"%qu-%qu", range.location, GrowingDDMaxRange(range) - 1];
...
- (GrowingHTTPMessage *)newMultiRangeResponse:(UInt64)contentLength
{
	...
	{
		GrowingDDRange range = [[ranges objectAtIndex:i] GrowingDDRangeValue];
...
- (void)sendResponseHeadersAndBody
{
	if ([GrowingHTTPResponse respondsToSelector:@selector(delayResponseHeaders)])
	{
		if ([GrowingHTTPResponse delayResponseHeaders])
		{
			return;
		}
	}
	...
	if ([GrowingHTTPResponse respondsToSelector:@selector(isChunked)])
	{
		isChunked = [GrowingHTTPResponse isChunked];
	}
	...
	if (!isChunked)
	{
		contentLength = [GrowingHTTPResponse contentLength];
	}
	...
	if (!isRangeRequest)
	{
		...
		if ([GrowingHTTPResponse respondsToSelector:@selector(status)])
		{
			status = [GrowingHTTPResponse status];
		}
		response = [[GrowingHTTPMessage alloc] initResponseWithStatusCode:status description:nil version:HTTPVersion1_1];
	...
	else
	{
		...
		if (!isRangeRequest)
		{
			...
			NSData *data = [GrowingHTTPResponse readDataOfLength:READ_CHUNKSIZE];
			...
			if ([data length] > 0)
			{
				...
				if (isChunked)
				{
					...
					{
						NSData *footer = [GrowingGCDAsyncSocket CRLFData];
						[asyncSocket writeData:footer withTimeout:TIMEOUT_WRITE_HEAD tag:HTTP_CHUNKED_RESPONSE_FOOTER];
					}
				}
				else
				{
					long tag = [GrowingHTTPResponse isDone] ? HTTP_RESPONSE : HTTP_PARTIAL_RESPONSE_BODY;
		...
		else
		{
			...
			if ([ranges count] == 1)
			{
				...
				[GrowingHTTPResponse setOffset:range.location];
				
				NSUInteger bytesToRead = range.length < READ_CHUNKSIZE ? (NSUInteger)range.length : READ_CHUNKSIZE;
				
				NSData *data = [GrowingHTTPResponse readDataOfLength:bytesToRead];
...
- (void)continueSendingStandardResponseBody
{
	...
	NSData *data = [GrowingHTTPResponse readDataOfLength:available];
...
- (void)continueSendingSingleRangeResponseBody
{
	...
	UInt64 offset = [GrowingHTTPResponse offset];
...
- (void)continueSendingMultiRangeResponseBody
{
	...
	GrowingDDRange range = [[ranges objectAtIndex:rangeIndex] GrowingDDRangeValue];
	...
	else
	{
		if (++rangeIndex < [ranges count])
		{
			...
			range = [[ranges objectAtIndex:rangeIndex] GrowingDDRangeValue];
...
- (NSObject<GrowingHTTPResponse> *)GrowingHTTPResponseForMethod:(NSString *)method URI:(NSString *)path
{
	...
	{
		return [[GrowingHTTPFileResponse alloc] initWithFilePath:filePath forConnection:self];
...
- (void)handleVersionNotSupported:(NSString *)version
{
	...
	GrowingHTTPMessage *response = [[GrowingHTTPMessage alloc] initResponseWithStatusCode:505 description:nil version:HTTPVersion1_1];
...
- (void)handleAuthenticationFailed
{
	...
	GrowingHTTPMessage *response = [[GrowingHTTPMessage alloc] initResponseWithStatusCode:401 description:nil version:HTTPVersion1_1];
	[response setHeaderField:@"Content-Length" value:@"0"];
...
- (void)handleInvalidRequest:(NSData *)data
{
	...
	GrowingHTTPMessage *response = [[GrowingHTTPMessage alloc] initResponseWithStatusCode:400 description:nil version:HTTPVersion1_1];
	[response setHeaderField:@"Content-Length" value:@"0"];
...
- (void)handleUnknownMethod:(NSString *)method
{
	...
	GrowingHTTPMessage *response = [[GrowingHTTPMessage alloc] initResponseWithStatusCode:405 description:nil version:HTTPVersion1_1];
	[response setHeaderField:@"Content-Length" value:@"0"];
	[response setHeaderField:@"Connection" value:@"close"];
...
- (void)handleResourceNotFound
{
	...
	GrowingHTTPMessage *response = [[GrowingHTTPMessage alloc] initResponseWithStatusCode:404 description:nil version:HTTPVersion1_1];
	[response setHeaderField:@"Content-Length" value:@"0"];
...
- (NSData *)preprocessResponse:(GrowingHTTPMessage *)response
{
	...
	if ([GrowingHTTPResponse respondsToSelector:@selector(httpHeaders)])
	{
		NSDictionary *responseHeaders = [GrowingHTTPResponse httpHeaders];
...
#pragma mark GrowingGCDAsyncSocket Delegate
...
- (void)socket:(GrowingGCDAsyncSocket *)sock didReadData:(NSData*)data withTag:(long)tag
{
		...
		else
		{
			...
			if (expectsUpload)
			{
				...
				else
				{
					...
					if (![NSNumber growing_parseString:(NSString *)contentLength intoUInt64:&requestContentLength])
					{
	...
	else
	{
		...
		else if (tag == HTTP_REQUEST_CHUNK_TRAILER)
		{
			...
			if (![data isEqualToData:[GrowingGCDAsyncSocket CRLFData]])
			{
				[self handleInvalidRequest:nil];
				return;
			}
...
#pragma mark GrowingHTTPResponse Notifications
...
- (void)die
{
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:GrowingHTTPConnectionDidDieNotification object:self];
}
...
@implementation GrowingHTTPConfig

@synthesize server;
@synthesize documentRoot;
@synthesize queue;

- (id)initWithServer:(GrowingHTTPServer *)aServer documentRoot:(NSString *)aDocumentRoot
{
	if ((self = [super init]))
	{
		server = aServer;
...
- (id)initWithServer:(GrowingHTTPServer *)aServer documentRoot:(NSString *)aDocumentRoot queue:(dispatch_queue_t)q
{
	if ((self = [super init]))
	{
		server = aServer;
