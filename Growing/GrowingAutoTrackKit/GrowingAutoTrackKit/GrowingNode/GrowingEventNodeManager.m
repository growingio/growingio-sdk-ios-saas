                   ...
                   eventType:(GrowingEventType)eventType
{
    ...
    BOOL(^checkBlock)(id<GrowingNode> node) = ^BOOL(id<GrowingNode> node) {
        ...
        if ([node growingNodeDonotTrackImp]&&
            (eventType ==GrowingEventTypeUIAddSubView||
            eventType ==GrowingEventTypeUISetAlpha||
            eventType ==GrowingEventTypeUISetHidden||
            eventType == GrowingEventTypeUIPageShow))
        {
                NSLog(@"growingAttributesDonotTrackImp%@", node);
                return NO;
        }

        if ([node respondsToSelector:@selector(growingEventProtocolCanTrackWithType:triggerNode:)])
        {
            @strongify(self);
            id<GrowingEventProtocol> eventNode = (id)node;
            
            return
            [eventNode growingEventProtocolCanTrackWithType:eventType
                                                triggerNode:self.triggerNode];
        }
        return YES;
    };
